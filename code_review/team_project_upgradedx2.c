...
void itoa(int n, char s[])/*функцията проверява дали числото е положително или е отрицателно, след което по формула запазва числата в стринг*/
{
    int i = 0, sign = n;

    if (n < 0)
        n = -n;
    do                          /*премаханти са всички броячи*/
    {
        s[i++] = n % 10 + '0';
        n /= 10;
    } while (n > 0);
...
void getReversed(char s[])    /*Функцията е пренаписана, така че само да обръща стринга*/
{
